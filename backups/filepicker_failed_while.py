...
def inputSentinelInt(From=0, To=999, message='Please pick a number: '):
    ...
    number = '-999999'  # dummy value
    # control imput to numer withing correct range
    condit1 = number.isnumeric() == False
    condit2 = int(From) > int(number) 
    condit3 = int(number) > int(number) > int(To)
    while condit1 or condit2 or condit3:
